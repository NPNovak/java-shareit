...
class BookingRepositoryTest {
    ...
    void fillingDB() {
        ...
        User user = userRepository.findById(1).orElseThrow(() -> new ValidationException("test"));
        for (int i = 1; i <= 5; ++i) {
            itemRepository.save(new Item(i, "item name " + i, "description " + i, true, user, null));
        }

        user = userRepository.findById(2).orElseThrow(() -> new ValidationException("test"));
        for (int i = 6; i <= 10; ++i) {
            itemRepository.save(new Item(i, "item name " + i, "description " + i, true, user, null));
        }

        Item item = itemRepository.findById(1).orElseThrow(() -> new ValidationException("test"));
        for (int i = 1; i <= 3; ++i) {
            LocalDateTime start = LocalDateTime.of(i, 1, 1, 1, 1, 1);
            LocalDateTime end = LocalDateTime.of(1 + i, 1, 1, 1, 1, 1);
            repository.save(new Booking(i, start, end, item, user, "APPROVED"));
        }
    ...
    void findFirstByItemIdAndItem_OwnerIdAndStartBeforeAndStatusOrderByStartDesc() {
        Booking lastBooking = repository
                .findFirstByItemIdAndItemOwnerIdAndStartBeforeAndStatusOrderByStartDesc(
                        1, 1, LocalDateTime.now(), "APPROVED");
